...
function theabd_render_vendor_store_hours_block( array $attributes, string $content, WP_Block $block ): string {
	...
	$dokan_days = function_exists( 'dokan_get_translated_days' ) ? dokan_get_translated_days() : array(
		'monday'    => __( 'Monday', 'theanother-blocks-for-dokan' ),
		'tuesday'   => __( 'Tuesday', 'theanother-blocks-for-dokan' ),
		'wednesday' => __( 'Wednesday', 'theanother-blocks-for-dokan' ),
		'thursday'  => __( 'Thursday', 'theanother-blocks-for-dokan' ),
		'friday'    => __( 'Friday', 'theanother-blocks-for-dokan' ),
		'saturday'  => __( 'Saturday', 'theanother-blocks-for-dokan' ),
		'sunday'    => __( 'Sunday', 'theanother-blocks-for-dokan' ),
	);
					...
					if ( ! empty( $current_day_schedule['opening_time'] ) && ! empty( $current_day_schedule['closing_time'] ) ) {
						echo esc_html(
							sprintf(
								...
								__( 'Today: %1$s - %2$s', 'theanother-blocks-for-dokan' ),
								$current_day_schedule['opening_time'],
								$current_day_schedule['closing_time']
							)
						);
					} else {
						echo esc_html__( 'Store hours vary. Please check detailed schedule.', 'theanother-blocks-for-dokan' );
					}
					?>
				</p>
			</div>
